...
This makes port 8080 available only to other containers on the Docker network and not the host. No one else on the network has access to SearXNG. We can now access SearXNG via Caddy at `https://localhost` and use this local instance as our search engine in Firefox!
